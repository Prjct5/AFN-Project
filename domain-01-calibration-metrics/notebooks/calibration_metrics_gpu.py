...
def get_user_input():
    ...
    print("\n" + "="*80)
    print("ARABIC FAKE NEWS DETECTION - ADVANCED CALIBRATION METRICS (GPU Optimized)")
    print("="*80)
    ...
    if torch.cuda.is_available():
        ...
        if gpu_memory_gb >= 24: # A100, RTX 4090, etc.
            suggested_batch = 64
        elif gpu_memory_gb >= 16: # RTX 4080, RTX 3080 Ti
            suggested_batch = 32
        elif gpu_memory_gb >= 12: # RTX 3060, RTX 3080
            suggested_batch = 32
        elif gpu_memory_gb >= 8: # RTX 2070, RTX 3070
            suggested_batch = 24
        else: # 6GB or less
            suggested_batch = 16
        print(f"Recommended batch size for your GPU ({gpu_memory_gb:.0f}GB): {suggested_batch}")
    
    try:
        batch_size_input = input(f" Batch size for inference (default {suggested_batch if torch.cuda.is_available() else 16}): ").strip()
        batch_size = int(batch_size_input) if batch_size_input else (suggested_batch if torch.cuda.is_available() else 16)
    except:
        batch_size = suggested_batch if torch.cuda.is_available() else 16
    ...
    # Ask whether to use mixed precision (FP16) for faster inference
    use_fp16 = False
    if torch.cuda.is_available():
        fp16_input = input(" Use mixed precision (FP16) for faster inference? (y/n, default y): ").strip().lower()
        use_fp16 = fp16_input != 'n'
...
def predict_with_probabilities(texts, tokenizer, model, device, batch_size=16, use_fp16=False):
    """Batch prediction returning raw probabilities (logits + softmax) with GPU optimization"""
    model.eval()
    ...
    processed_texts = [preprocess_arabic_text(str(t)) for t in texts]
    
    # Calculate optimal batch size based on GPU memory if needed
    if batch_size <= 0:
        if device.type == 'cuda':
    ...
    with torch.no_grad():
        for i in tqdm(range(0, len(processed_texts), batch_size), desc="Running inference (GPU)"):
            ...
            # Move to CPU for numpy conversion (faster than GPU->numpy directly)
            ...
            if device.type == 'cuda' and (i + batch_size) % (batch_size * 100) == 0:
                torch.cuda.empty_cache()
...
<head>
    ...
    <title>Advanced Calibration Metrics - ArBERT Evaluation Report (GPU Accelerated)</title>
    <style>
...
<body>
    <div class="container">
        <h1> Advanced Calibration & Statistical Metrics Report (GPU Accelerated)</h1>
        <p>Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</p>
        <p><strong>Model:</strong> zhafyz/arabic-fake-news-arbert-afnd | <strong>Device:</strong> {'CUDA (GPU)' if torch.cuda.is_available() else 'CPU'}</p>
        <p><strong>GPU Info:</strong> {torch.cuda.get_device_name(0) if torch.cuda.is_available() else 'N/A'}</p>
        <p><strong>Mixed Precision (FP16):</strong> {'Enabled' if config.get('use_fp16', False) else 'Disabled'}</p>
    ...
    html_content += f"""
        ...
        <div class="footer">
            <p>Generated by ArBERT Advanced Calibration Evaluation System (GPU Optimized)</p>
...
def main():
    ...
    try:
        ...
        if device.type == 'cuda':
            print(f"Model VRAM usage: {torch.cuda.memory_allocated(0) / 1e9:.2f} GB")
        ...
        if device.type == 'cuda':
            # Use torch.jit to optimize if possible (optional)
            # model = torch.jit.optimize_for_inference(model) # Uncomment if needed
            pass
    ...
    print(f"\nRunning inference with probability extraction (Batch size: {config['batch_size']})...")
    ...
    prob_fake, confidences = predict_with_probabilities(
        texts, tokenizer, model, device, config['batch_size'], config.get('use_fp16', False)
    )
    ...
    if device.type == 'cuda':
        ...
        print("GPU PERFORMANCE SUMMARY")
        print("="*80)
        print(f"Peak GPU Memory Used: {torch.cuda.max_memory_allocated(0) / 1e9:.2f} GB")
